...
class SAM2Base(torch.nn.Module):
    ...
    def _forward_sam_heads(
        ...
        if multimask_output and self.samurai_mode:
            if self.kf_mean is None and self.kf_covariance is None or self.stable_frames == 0:
                ...
                if ious[0][best_iou_inds[0]] < self.stable_ious_threshold:
                    ...
                    self.kf_mean, self.kf_covariance = self.kf.update(self.kf_mean, self.kf_covariance, self.kf.xyxy_to_xyah(high_res_multibboxes[best_iou_inds[0].item()]))
        ...
        return (
            ...
            obj_ptr,
            object_score_logits,
            ious[0][best_iou_inds[0]] if isinstance(best_iou_inds, torch.Tensor) else ious[0][best_iou_inds],
            ...
            if self.samurai_mode:
                ...
                if frame_idx > 1:  # Ensure we have previous frames to evaluate
                    for i in range(frame_idx - 1, 1, -1):  # Iterate backwards through previous frames
                        ...
                        if iou_score.max().item() > self.memory_bank_iou_threshold and \
                           obj_score.max().item() > self.memory_bank_obj_score_threshold and \
                           (kf_score is None or kf_score.max().item() > self.memory_bank_kf_score_threshold):
